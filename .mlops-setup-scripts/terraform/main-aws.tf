...
resource "aws_iam_policy" "my-s3-read-policy" {
  name        = "my-mlp-terraform-state-manage"
  description = "S3 bucket access policy for managing terraform state for my-mlp"
...
resource "aws_iam_user" "s3_bucket_cicd_user" {
  name = "my-mlp-terraform-cicd-user"
}
...
resource "aws_s3_bucket" "cicd_terraform_state" {
  bucket = "my-mlp-cicd-setup-tfstate"
...
resource "aws_dynamodb_table" "cicd_terraform_state_lock" {
  name           = "my-mlp-cicd-setup-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
